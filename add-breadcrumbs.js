...
function getImportPath(filePath) {
  const relativePath = path.relative(
    ...
    'docs/components/AutoBreadcrumb'
  );
  ...
  try {
    ...
    if (hasFrontmatter) {
      ...
      while (
        ...
        beforeHeading[beforeHeading.length - 1].trim() === ''
      ) {
        beforeHeading.pop();
      }
...
async function main() {
  ...
  for (const filePath of allFiles) {
    ...
    if (result === true) {
      ...
      result === false &&
      fs.readFileSync(filePath, 'utf8').includes('AutoBreadcrumb')
    ) {
      skippedCount++;
    } else {
      errorCount++;
  ...
  if (processedCount > 0) {
    ...
    console.log(
      '💡 You can now navigate to any page in /agents/, /chat-apps/, /network/, or /protocol/ to see the breadcrumbs.'
    );
  }
}
