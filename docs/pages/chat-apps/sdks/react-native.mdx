...
// Send with push notification control (v5.1.0+)
await dm.send('Silent message', { shouldPush: false });

// 5. List, stream, and sync
// List existing conversations with advanced filtering (v5.1.0+)
await client.conversations.list(['allowed']);

// List with time-based filtering
