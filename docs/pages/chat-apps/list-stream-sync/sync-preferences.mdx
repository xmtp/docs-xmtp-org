...
Stream preference updates in real-time. This allows your app to react immediately when preferences (consent states, HMAC keys) are updated from another installation.

:::code-group

```js [Browser]
// Stream all preference updates
const stream = await client.preferences.streamPreferences({
  onValue: (preferenceUpdate) => {
    console.log("Preference updated:", preferenceUpdate);
  },
  onError: (error) => {
...
for await (const preferenceUpdate of stream) {
  console.log("Preference updated:", preferenceUpdate);
}
```
