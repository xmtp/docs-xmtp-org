...
Listen for real-time preference updates across installations. This is useful for keeping your UI in sync when preferences change on another device.
...
client.preferences.streamPreferenceUpdates().collect { preferenceType ->
    when (preferenceType) {
        PreferenceType.HMAC_KEYS -> {
            println("HMAC keys updated")
            // Refresh push notification configuration
        }
    }
}
```
...
| Type | Description |
| ---- | ----------- |
| `HMAC_KEYS` | HMAC keys for push notification topic encryption were updated |

## Get conversation consent state

Get the consent state for a specific conversation by its group ID:

:::code-group

```kotlin [Kotlin]
// Get the consent state for a specific conversation
val consentState = client.preferences.conversationState(groupId)

when (consentState) {
    ConsentState.ALLOWED -> println("Conversation is allowed")
    ConsentState.DENIED -> println("Conversation is denied")
    ConsentState.UNKNOWN -> println("Consent state is unknown")
}
```
