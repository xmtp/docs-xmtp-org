...
- When a user is added to a group, `await isActive()` returns `true` for that user  
- When a user is removed from a group, `await isActive()` returns `false` for that user

In Browser SDK, `isActive()` is an async method that must be awaited. You can use the result to filter conversations or create separate sections for "archived" or "inactive" conversations.
