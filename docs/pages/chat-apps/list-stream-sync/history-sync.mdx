...
:::warning[Breaking Change in Browser SDK v7.0.0+]
**History sync is now manual.** Automatic sending of sync requests on new installations has been removed. You must call `sendSyncRequest()` explicitly after creating a client on a new installation to trigger a history sync.
:::

History sync is available but requires manual activation. When [creating a client](/chat-apps/core-messaging/create-a-client), the `historySyncUrl` client option is dynamically set to a default history sync server URL based on your `env` client option setting.
