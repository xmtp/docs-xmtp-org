...
| `elements` | `[BackupElementSelectionOption.Consent, BackupElementSelectionOption.Messages]` | What to include in the archive: `messages`, `consent`, or both |
...
```js [Browser]
// With default options (syncs consent and messages to default server)
await client.sendSyncRequest();

// With custom options
await client.sendSyncRequest(
  { elements: [BackupElementSelectionOption.Consent] },            // options
...
```js [Node]
// List archives from the last 30 days
const archives = client.listAvailableArchives(30);
```
