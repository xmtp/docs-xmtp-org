...
History sync is enabled by default. When [creating a client](/chat-apps/core-messaging/create-a-client), XMTP automatically uses the appropriate history sync server URL based on your `env` client option setting.

- When `env` is set to `dev`, XMTP uses `https://message-history.dev.ephemera.network/`
- When `env` is set to `production`, XMTP uses `https://message-history.production.ephemera.network`
...
You can choose to [run your own server](https://github.com/xmtp/xmtp-message-history-server) and configure your client to use it with the new sync archive methods.
