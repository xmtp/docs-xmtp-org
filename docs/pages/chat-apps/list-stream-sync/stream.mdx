...
**Error Handling**: Stream errors are handled through the `onError` callback function rather than throwing exceptions. This allows for more flexible error handling and prevents unhandled promise rejections.
:::
