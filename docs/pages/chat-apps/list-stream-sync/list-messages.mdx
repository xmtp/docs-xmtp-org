...
| `excludeContentTypes` / `excludedContentTypes` | Array | Content types to exclude (e.g., reactions, read receipts, replies) |
| `excludeSenderInboxIds` | Array of strings | Inbox IDs whose messages should be excluded from results |
...
groupMessages.forEach(message => {
  ...
  message.reactions?.forEach(reaction => {
    console.log(`Reaction: ${reaction.content.content}`);
  });
});
```
