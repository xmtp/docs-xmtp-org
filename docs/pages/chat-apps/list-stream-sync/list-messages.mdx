...
| `sortBy` | `MessageSortBy` / `string` | Sort criterion: `sent_at` (default, by sent timestamp) or `inserted_at` (by insertion timestamp) |
| `insertedAfterNs` | `Int64` / `Long` / `bigint` | Filter messages inserted into local database after this timestamp (nanoseconds) |
| `insertedBeforeNs` | `Int64` / `Long` / `bigint` | Filter messages inserted into local database before this timestamp (nanoseconds) |
