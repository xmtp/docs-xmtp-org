...
## Enriched messages method parameters

For `enrichedMessages()`, the available parameters are more limited but include new options:

| Parameter           | Type                               | Description                                                                 |
| ------------------- | ---------------------------------- | --------------------------------------------------------------------------- |
| `limit`             | `Int` / `Long` / `bigint`          | Maximum number of messages to return                                        |
| `beforeNs`          | `Int64` / `Long` / `bigint`        | Filter messages sent before this timestamp (nanoseconds)                    |
| `afterNs`           | `Int64` / `Long` / `bigint`        | Filter messages sent after this timestamp (nanoseconds)                     |
| `direction`         | `SortDirection` / `string`         | Sort order: `descending` (default, newest first) or `ascending` (oldest first) |
| `excludeSenderInboxIds` | Array of strings               | Inbox IDs whose messages should be excluded                                 |
| `deliveryStatus`    | `EnrichedMessageDeliveryStatus`    | Filter by delivery status for enriched messages                             |
| `insertedAfterNs`   | `Int64` / `Long` / `bigint`        | Filter messages inserted after this timestamp (nanoseconds)                 |
| `insertedBeforeNs`  | `Int64` / `Long` / `bigint`        | Filter messages inserted before this timestamp (nanoseconds)                |
| `sortBy`            | `EnrichedMessageSortBy`            | Sort by sent time or insertion time for enriched messages                   |
