...
| `excludeContentTypes` / `excludedContentTypes` | Array | Content types to exclude (e.g., reactions, read receipts). **New in v5.1.0** |
| `excludeSenderInboxIds` | Array of strings | Inbox IDs whose messages should be excluded. **New in v5.1.0** |
...
```tsx [React Native]
// Retrieve messages with enriched data from a group
const groupMessages = await group.messagesWithReactions();

// Retrieve messages with enriched data from a DM
const dmMessages = await dm.messagesWithReactions();
...
groupMessages.forEach((message) => {
  // Check if the message is a reply
  if (message.contentTypeId.startsWith('xmtp.org/reply:')) {
...
const messages = await group.messages({
  ...
  direction: 'DESCENDING', // Sort direction
  excludeContentTypes: ['xmtp.org/reaction:1.0'], // Exclude content types
  excludeSenderInboxIds: ['inbox_id_1'], // Exclude senders
});

// Using messagesWithReactions() for enriched data with filtering
const enrichedMessages = await group.messagesWithReactions({
  afterNs: 1738620126404999936,
  limit: 50,
  excludeSenderInboxIds: ['blocked_user_inbox_id'],
});
