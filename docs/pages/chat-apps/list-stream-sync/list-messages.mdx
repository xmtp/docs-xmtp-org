...
|-----------|------|-------------|
| `limit` | `Int` / `Long` / `bigint` / `number` | Maximum number of messages to return |
| `beforeNs` / `sentBeforeNs` | `Int64` / `Long` / `bigint` / `number` | Filter messages sent before this timestamp (nanoseconds) |
| `afterNs` / `sentAfterNs` | `Int64` / `Long` / `bigint` / `number` | Filter messages sent after this timestamp (nanoseconds) |
| `direction` | `SortDirection` / `string` | Sort order: `descending` / `'DESCENDING'` (default, newest first) or `ascending` / `'ASCENDING'` (oldest first) |
| `deliveryStatus` | `MessageDeliveryStatus` / `string` | Filter by delivery status: `all` (default), `published`, `unpublished`, or `failed` |
| `excludeContentTypes` / `excludedContentTypes` | Array of strings | Content types to exclude (e.g., `'xmtp.org/reaction:1.0'`, `'xmtp.org/readReceipt:1.0'`). **New in React Native v5.1.0** |
| `excludeSenderInboxIds` | Array of strings | Inbox IDs whose messages should be excluded. **New in React Native v5.1.0** |
| `insertedBeforeNs` | `Int64` / `Long` / `bigint` / `number` | Filter messages inserted into the local database before this timestamp (nanoseconds) |
| `insertedAfterNs` | `Int64` / `Long` / `bigint` / `number` | Filter messages inserted into the local database after this timestamp (nanoseconds) |
| `sortBy` | `SortBy` / enum / `string` | Sort by `SENT_TIME` / `'SENT_TIME'` (default, when message was sent) or `INSERTED_TIME` / `'INSERTED_TIME'` (when message was added to local database) |
...
const filteredMessages = await group.messages({
  ...
  afterNs: lastCheckTimestamp,
  direction: 'ASCENDING',
  excludeContentTypes: ['xmtp.org/reaction:1.0'], // Exclude reactions
  excludeSenderInboxIds: ['inbox-id-to-exclude'], // Exclude messages from specific senders
});
...
const dmMessages = await dm.messages({ 
  limit: 50,
  excludeContentTypes: ['xmtp.org/readReceipt:1.0'], // Exclude read receipts
});
```
...
```tsx [React Native]
// Using messages() with comprehensive filtering
const messages = await group.messages({
  limit: 100,
  afterNs: 1738620126404999936, // Filter by time
  direction: 'DESCENDING', // Sort order
  excludeContentTypes: [
    'xmtp.org/reaction:1.0',
    'xmtp.org/readReceipt:1.0'
  ], // Exclude content types
  excludeSenderInboxIds: ['inbox_id_1', 'inbox_id_2'], // Exclude senders
});

// Get text messages only (exclude all other content types)
const textOnlyMessages = await group.messages({
  excludeContentTypes: [
    'xmtp.org/reaction:1.0',
