...
## Get message metadata without a client instance

Use this static method to retrieve message metadata without instantiating a complete client. This is useful for background services, notification handlers, and scenarios where you need quick access to message information without the overhead of a full client connection.

The method returns the newest message metadata for the specified group, including the message ID, timestamps, sender information, and content type.

:::code-group

```kotlin [Kotlin]
// Get the newest message metadata for a group without a full client
val metadata = Client.getNewestMessageMetadata(
    dbPath = "/path/to/database.db3",
    encryptionKey = encryptionKeyBytes,
    groupId = groupId
)

// Access metadata fields
println("Message ID: ${metadata.id}")
println("Sent at: ${metadata.sentAtNs}")
println("Sender inbox ID: ${metadata.senderInboxId}")
println("Content type: ${metadata.contentType}")
println("Expires at: ${metadata.expiresAtNs}")
```

```swift [Swift]
// Get the newest message metadata for a group without a full client
let metadata = try Client.getNewestMessageMetadata(
...
// Access metadata fields
print("Message ID: \(metadata.id)")
print("Sent at: \(metadata.sentAtNs)")
print("Sender inbox ID: \(metadata.senderInboxId)")
print("Content type: \(metadata.contentType)")
print("Expires at: \(metadata.expiresAtNs ?? 0)")
```

:::

## Access message expiration timestamps

All decoded message types now include an optional `expiresAtNs` field, which contains the expiration timestamp in nanoseconds. This field enables ephemeral messaging features and time-limited communications.

You can use this feature to build ephemeral messaging, auto-deleting content, and time-sensitive messaging patterns. Unlike [disappearing messages](/chat-apps/core-messaging/disappearing-messages), which are automatically cleaned up by the SDK, the `expiresAtNs` field simply provides expiration metadata that your app can use to implement custom expiration behavior.

:::code-group

```tsx [Browser]
// Access message expiration timestamp
const messages = await group.messages();
...
messages.forEach((message) => {
  if (message.expiresAtNs) {
    ...
    if (BigInt(now) > message.expiresAtNs) {
...
messages.forEach((message) => {
  if (message.expiresAtNs) {
    const expirationTime = new Date(message.expiresAtNs / 1_000_000);
    console.log(`Message expires at: ${expirationTime}`);

    // Check if message is expired
    const now = Date.now() * 1_000_000; // Convert to nanoseconds
    if (now > message.expiresAtNs) {
      console.log('Message has expired');
    }
  }
});
...
messages.forEach { message ->
    message.expiresAtNs?.let { expiresAt ->
        ...
        // Check if message is expired
        val now = Clock.System.now().toEpochMilliseconds() * 1_000_000
        if (now > expiresAt) {
