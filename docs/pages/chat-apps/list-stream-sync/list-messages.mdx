...
| `excludeContentTypes` / `excludedContentTypes` | Array | Content types to exclude (e.g., reactions, read receipts). Not available for `enrichedMessages()` |
| `excludeSenderInboxIds` | Array of strings | Inbox IDs whose messages should be excluded. Not available for `enrichedMessages()` |
...
Each message includes reaction information:

- **`reactions`**: List of reaction messages associated with the message (available in Swift and Kotlin)
- **`hasReactions`**: Boolean indicating if the message has any reactions (Kotlin only)
- **`reactionCount`**: Total number of reactions on the message (Kotlin only)

In Swift, check if `reactions` is nil or use `reactions?.count` to get the number of reactions.
...
for message in groupMessages {
  if let reactions = message.reactions {
    print("This message has \(reactions.count) reactions")

    // Access individual reactions
...
groupMessages.forEach { message ->
  if (message.hasReactions) {
    println("This message has ${message.reactionCount} reactions")

    // Access individual reactions
    message.reactions.forEach { reaction ->
      val reactionContent = reaction.content<Reaction>()
      println("Reaction: ${reactionContent.content} from ${reactionContent.reference}")
    }
  }
}
```
