...
- **Externally Owned Account (EOA)**: Use an existing Ethereum wallet.
...
If your users already have Ethereum wallets (MetaMask, Coinbase Wallet, etc.), you can create a signer from their Externally Owned Account. The signer must have 3 properties: the account type, a function that returns the account identifier, and a function that signs messages.
...
If your users have ERC-1271 compatible smart contract wallets (Safe, Coinbase Smart Wallet, etc.), the SCW signer has the same 3 required properties as the EOA signer, but also requires a function that returns the chain ID and an optional function that returns the block number to verify signatures against. If a function is not provided to retrieve the block number, the latest block number will be used.
