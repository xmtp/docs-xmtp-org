...
XMTP SDKs support message signing with Ethereum via Externally Owned Accounts (EOAs), Ethereum Smart Contract Wallets (SCWs), orpasskeys. All SDK clients accept a signer object (or instance), which provides a method for signing messages.
