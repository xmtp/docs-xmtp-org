...
// For custom content types, encode the content and use send with optimistic parameter
...
const encodedContent = await conversation.encodeContent(customContent, contentType);
await conversation.send(encodedContent, true);
```
