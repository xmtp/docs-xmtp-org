...
### Control when a message is published

For more control over when a prepared message is published, use the `noSend` parameter. When `noSend` is `true`, the message won't be published until you explicitly call `publishMessage(messageId)`. This is particularly useful for [remote attachments](/chat-apps/content-types/attachments) where an upload might fail.

:::code-group

```kotlin [Kotlin]
// Prepare message without auto-publishing
val messageId = conversation.prepareMessage(
    content = content,
    noSend = true
)

// Later, publish the specific message
...
```js [Browser]
// Built-in content types (text, reactions, replies, attachments, etc.) are automatically supported
// For unsupported custom content types, use the fallback
try {
  // Try to decode the message content
  const content = message.content;
} catch (error) {
  // If decoding fails, it's an unsupported content type
