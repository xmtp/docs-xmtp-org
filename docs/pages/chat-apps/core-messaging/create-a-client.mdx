...
const advancedClient = await Client.create(signer, {
  env: 'production',
  ...
  forkRecoveryOptions: {
    ...
    workerIntervalNs: 1000000000, // 1 second
  },
});
```
...
/**
 * Fork recovery policy for handling group forks
 */
type ForkRecoveryPolicy = 'none' | 'all' | 'groups';

/**
 * Configuration options for fork recovery
 */
...
// Client build with advanced options
const advancedClient = await Client.build(identity, {
  env: 'production',
  dbEncryptionKey: keyBytes,
  forkRecoveryOptions: {
    enableRecoveryRequests: 'groups',
