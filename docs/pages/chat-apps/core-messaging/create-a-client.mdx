...
Check if the client is registered with the XMTP network. This is useful when using `disableAutoRegister: true` in [client options](#configure-an-xmtp-client) to manually control the registration process.
