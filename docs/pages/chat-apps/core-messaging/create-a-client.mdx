...
type ForkRecoveryPolicy = 'none' | 'all' | 'groups';

type ForkRecoveryOptions = {
  /**
  ...
  /**
   * Worker interval in nanoseconds
   */
...
Fork recovery options help handle group conversation forks, which can occur when group state becomes inconsistent. Configure these options to control how your app responds to and recovers from group forks:
...
const client = await Client.create(signer, {
  ...
  forkRecoveryOptions: {
    enableRecoveryRequests: 'all', // Enable recovery for all groups
    disableRecoveryResponses: false, // Allow responding to recovery requests
    workerIntervalNs: 5000000000, // Check for recovery every 5 seconds
  },
});

// Or configure for specific groups only
const clientWithSelectiveRecovery = await Client.create(signer, {
  env: 'production', 
  dbEncryptionKey: keyBytes,
  forkRecoveryOptions: {
    enableRecoveryRequests: 'groups', // Only recover specific groups
...
**Fork Recovery Policy Options:**
- `'none'`: Disable recovery requests entirely
- `'all'`: Enable recovery for all groups (default recommended)
- `'groups'`: Enable recovery only for groups specified in `groupsToRequestRecovery`
