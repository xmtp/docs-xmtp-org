...
```tsx [React Native]
import type { ContentCodec, ForkRecoveryOptions } from '@xmtp/react-native-sdk';
...
### Configure Fork Recovery Options (React Native SDK)

The React Native SDK supports fork recovery options to handle group forks gracefully. These options help maintain group consistency when network partitions or other issues occur.
...
type ForkRecoveryOptions = {
   ...
   */
  enableRecoveryRequests?: 'none' | 'all' | 'groups';
  /**
...
const client = await Client.create(signer, {
  ...
  forkRecoveryOptions: {
    ...
    workerIntervalNs: 5000000000, // 5 seconds
  },
});
```
