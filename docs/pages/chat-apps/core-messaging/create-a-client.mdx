 ...
 * database.
 *
 * You can provide either a Uint8Array (32 bytes) or a hex string.
 */
const dbEncryptionKey = getRandomValues(new Uint8Array(32));
// Or as a hex string: const dbEncryptionKey = '0x' + Buffer.from(getRandomValues(new Uint8Array(32))).toString('hex');
...
type ClientOptions = {
   ...
   */
  dbEncryptionKey?: Uint8Array | string;
  /**
