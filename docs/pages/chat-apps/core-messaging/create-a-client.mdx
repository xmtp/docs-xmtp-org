...
Client.create(signer, {
  ...
  gatewayHost: 'your-custom-gateway.com', // Optional: specify custom gateway host
  forkRecoveryOptions: { // Optional: configure fork recovery behavior
    enableRecoveryRequests: 'groups', // 'none' | 'all' | 'groups'
    groupsToRequestRecovery: ['group-id-1', 'group-id-2'], // required when enableRecoveryRequests is 'groups'
    disableRecoveryResponses: false, // Optional: disable recovery responses
    workerIntervalNs: 60000000000, // Optional: worker interval in nanoseconds
  },
});
```
