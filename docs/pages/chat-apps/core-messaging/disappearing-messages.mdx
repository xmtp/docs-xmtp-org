...
for await (const message of stream) {
  ...
  // Update your UI to remove this message
}
...
### Stream message deletion IDs (deprecated)

:::warning[Deprecated]
The `streamMessageDeletions` method is deprecated in favor of `streamDeletedMessages`. Use `streamDeletedMessages` for new implementations.
:::

For backwards compatibility, you can still use `streamMessageDeletions` to return a stream of deleted message IDs:
