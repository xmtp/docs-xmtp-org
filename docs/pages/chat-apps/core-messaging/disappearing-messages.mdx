...
The `streamMessageDeletions` method that returns message IDs is deprecated. Use `streamDeletedMessages` instead to get the full deleted message objects.
:::

:::code-group

```tsx [Browser]
// Stream deleted messages (recommended)
...
// Deprecated: Stream message deletion IDs only
const idStream = await client.conversations.streamMessageDeletions();
for await (const messageId of idStream) {
