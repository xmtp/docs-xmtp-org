...
XMTP provides two methods to stream message deletions:

- `streamDeletedMessages()`: Returns a stream of the actual deleted messages (recommended)
- `streamMessageDeletions()`: Returns a stream of deleted message IDs (deprecated)

Both methods create a local stream that doesn't require network sync and will notify you when messages have been deleted from local storage.

:::code-group

```tsx [Browser]
// Stream deleted messages (recommended approach)
const deletedStream = await client.conversations.streamDeletedMessages({
  onValue: (message) => {
...
for await (const deletedMessage of deletedStream) {
  console.log('Deleted message:', deletedMessage);
  // Update your UI to remove the message
}

// Legacy approach: Stream message deletion IDs (deprecated)
