...
Each installation has a unique key that can be used to sign and verify messages. This is useful for proving that a message came from a specific installation.
...
```kotlin [Kotlin]
// Sign a message with the current installation's key
...
Verify that a signature was created by the current installation:

:::code-group

```kotlin [Kotlin]
// Verify a signature from the current installation
val isValid = client.verifySignature(message, signature)
...
Verify that a signature was created by a specific installation ID:

:::code-group

```kotlin [Kotlin]
// Verify a signature from a specific installation
val isValid = client.verifySignatureWithInstallationId(
    message = message,
    signature = signature,
