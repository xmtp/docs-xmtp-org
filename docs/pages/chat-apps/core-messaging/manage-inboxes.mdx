...
const backend = await createBackend({ env: "production" }); // optional, defaults to "dev"
...
await Client.revokeInstallations(signer, inboxId, toRevokeInstallationBytes, backend);
```
