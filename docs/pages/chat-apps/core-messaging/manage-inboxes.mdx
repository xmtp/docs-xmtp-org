...
To prevent users from reaching the 256-update limit, XMTP enforces a **10-installation limit per inbox**. When an inbox reaches 10 active installations, the user must revoke at least one installation before adding a new one. This limit serves two purposes:
