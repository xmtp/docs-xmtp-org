...
### Check installation key package statuses

For advanced debugging, you can check the key package status of installations. Key packages are cryptographic material used for secure message delivery. This is useful for diagnosing issues with message delivery to specific installations.
...
// Check key package statuses for installations
const statuses = await client.debugInformation.getKeyPackageStatuses(installationIds);

for (const [installationId, status] of statuses.statuses) {
...
// Check key package statuses for installations (static method)
let statuses = try await Client.keyPackageStatusesForInstallationIds(
    installationIds: installationIds.map { Data(hexString: $0) },
    env: .production
)

for (installationId, status) in statuses {
    print("Installation: \(installationId)")
    print("Lifetime: \(status.lifetime)")
    print("Validation error: \(status.validationError ?? "none")")
}
```

:::

### Verify address and installation authorization (Node SDK only)

The Node SDK provides static methods to verify if an address or installation is authorized for a specific inbox without creating a client. These are useful for server-side validation.

```js [Node]
import { Client, IdentifierKind } from "@xmtp/node-sdk";

// Check if an address is authorized for an inbox
const isAddressAuthorized = await Client.isAddressAuthorized(
  inboxId,
  "0xADDRESS_TO_CHECK",
  "production" // optional, defaults to "dev"
);
...
if (isAddressAuthorized) {
  console.log("Address is authorized for this inbox");
} else {
  console.log("Address is NOT authorized for this inbox");
}

// Check if an installation is authorized for an inbox
const isInstallationAuthorized = await Client.isInstallationAuthorized(
  inboxId,
  installationIdBytes, // Uint8Array
  "production" // optional, defaults to "dev"
);

if (isInstallationAuthorized) {
  console.log("Installation is authorized for this inbox");
} else {
  console.log("Installation is NOT authorized for this inbox");
}
```
...
These utility functions allow you to work with inbox IDs without requiring an active client instance.

### Generate an inbox ID

Generate the inbox ID that would be created for a given identifier. This is useful for predicting what inbox ID an identity will have before creating a client.

:::code-group

```js [Browser]
import { generateInboxId, IdentifierKind } from "@xmtp/browser-sdk";

// Generate the inbox ID for an Ethereum address
...
```js [Node]
import { generateInboxId, IdentifierKind } from "@xmtp/node-sdk";
...
### Look up an inbox ID by identifier

Look up the inbox ID associated with an identifier on the network. Returns `undefined` if the identifier has not been registered with XMTP.
...
```js [Node]
import { getInboxIdForIdentifier, IdentifierKind } from "@xmtp/node-sdk";
...
You can change which identity serves as the recovery identity for an inbox. The recovery identity is used to authorize important identity actions like removing identities and revoking installations.

:::warning[Important]

Only the current recovery identity can authorize this change. The new recovery identity must already be associated with the inbox before it can be designated as the recovery identity.
...
```js [Browser]
// Change the recovery identity to another identity associated with this inbox
await client.changeRecoveryIdentifier({
  kind: IdentifierKind.Ethereum,
  identifier: "0xNEW_RECOVERY_ADDRESS",
});
```
...
## Advanced: Custom signature flows

:::warning[Advanced use case]

These methods are for advanced developers who need fine-grained control over the signature flow. Most applications should use the standard methods like `unsafe_addAccount()`, `removeAccount()`, `revokeInstallations()`, and `changeRecoveryIdentifier()` which handle signatures automatically.

Use these methods only if you need to:
- Integrate with wallets that can't provide signatures synchronously
- Implement custom signature collection UIs
- Handle signatures across different contexts or processes

:::

The custom signature flow differs between SDKs:

**Browser SDK:**
1. Call `unsafe_*SignatureText()` to get `{signatureText, signatureRequestId}`
2. Sign the text with your wallet
3. Call `unsafe_applySignatureRequest(safeSigner, signatureRequestId)`

**Node SDK:**
1. Call `unsafe_*SignatureRequest()` to get a `SignatureRequestHandle`
2. Sign the text and add the signature with `unsafe_addSignature(handle, signatureBytes)`
3. Call `unsafe_applySignatureRequest(handle)`

**Mobile SDKs (React Native, Kotlin, Swift):**
1. Call `ffi*SignatureText()` or `ffiSignatureRequest()` to get signature text
2. Sign with ECDSA or SCW and call `ffiAddEcdsaSignature()` or `ffiAddScwSignature()`
3. Call `ffiApplySignature()` or `ffiApplySignatureRequest()`

### Create inbox signature request

Generate the signature request needed for creating a new inbox:

:::code-group

```js [Browser]
import { toSafeSigner } from "@xmtp/browser-sdk";

// Step 1: Generate the signature text
const { signatureText, signatureRequestId } =
  await client.unsafe_createInboxSignatureText();

// Step 2: Sign with your wallet (example using a custom signer)
const safeSigner = toSafeSigner(yourWalletSigner);
...
```js [Node]
// Step 1: Generate the signature request
const signatureRequest = await client.unsafe_createInboxSignatureRequest();

// Step 2: Get the signature text and sign it with your wallet
const signatureText = signatureRequest.signatureText();
...
// Step 4: Apply the signature request
await client.unsafe_applySignatureRequest(signatureRequest);
```

```tsx [React Native]
// Step 1: Generate the signature text
const signatureText = await client.ffiCreateSignatureText();

// Step 2: Sign with your wallet (ECDSA example)
const signatureBytes = await yourWallet.signMessage(signatureText);

// Step 3: Add the ECDSA signature
await client.ffiAddEcdsaSignature(signatureBytes);

// Step 4: Apply the signature
await client.ffiApplySignature();
```

```kotlin [Kotlin]
// Step 1: Get the signature request
val signatureRequest = client.ffiSignatureRequest()

// Step 2: Handle the signature with your signing key
if (signatureRequest != null) {
    client.handleSignature(signatureRequest, signingKey)

    // Step 3: Apply the signature request
    client.ffiApplySignatureRequest(signatureRequest)
}
```

```swift [Swift]
// Step 1: Get the signature request
if let signatureRequest = client.ffiSignatureRequest() {
    // Step 2: Handle the signature with your signing key
    ...
    // Step 3: Apply the signature request
    try await client.ffiApplySignatureRequest(signatureRequest: signatureRequest)
}
```

:::

### Add account signature request

Generate the signature request needed for adding an account to an inbox:

:::code-group

```js [Browser]
import { IdentifierKind, toSafeSigner } from "@xmtp/browser-sdk";

// Step 1: Generate the signature text for adding an account
const { signatureText, signatureRequestId } =
  await client.unsafe_addAccountSignatureText(
    {
      kind: IdentifierKind.Ethereum,
...
// Step 2: Get the signature text and sign it with the NEW account's wallet
const signatureText = signatureRequest.signatureText();
const signatureBytes = await newAccountWallet.signMessage(signatureText);
...
### Remove account signature request

Generate the signature request needed for removing an account from an inbox:
...
// Step 1: Generate the signature text for removing an account
const { signatureText, signatureRequestId } =
  await client.unsafe_removeAccountSignatureText({
    kind: IdentifierKind.Ethereum,
    identifier: "0xACCOUNT_TO_REMOVE",
  });

// Step 2: Sign with the recovery account's wallet
const recoverySafeSigner = toSafeSigner(recoveryWalletSigner);

// Step 3: Apply the signature
await client.unsafe_applySignatureRequest(recoverySafeSigner, signatureRequestId);
```

```js [Node]
import { IdentifierKind } from "@xmtp/node-sdk";

// Step 1: Generate the signature request for removing an account
const signatureRequest = await client.unsafe_removeAccountSignatureRequest({
  kind: IdentifierKind.Ethereum,
  identifier: "0xACCOUNT_TO_REMOVE",
});

// Step 2: Get the signature text and sign it with the recovery account's wallet
const signatureText = signatureRequest.signatureText();
const signatureBytes = await recoveryWallet.signMessage(signatureText);
...
### Revoke installations signature request

Generate the signature request needed for revoking specific installations:
...
const inboxState = await client.preferences.fetchInboxState();
const installationsToRevoke = inboxState.installations
  .filter(i => /* your filter logic */)
  .map(i => i.bytes);

// Step 1: Generate the signature text
const { signatureText, signatureRequestId } =
  await client.unsafe_revokeInstallationsSignatureText(installationsToRevoke);

// Step 2: Sign with your wallet
const safeSigner = toSafeSigner(walletSigner);
...
const signatureRequest =
  await client.unsafe_revokeInstallationsSignatureRequest(installationsToRevoke);
...
### Revoke all other installations signature request

Generate the signature request needed for revoking all installations except the current one:
...
// Step 1: Generate the signature text
const { signatureText, signatureRequestId } =
  await client.unsafe_revokeAllOtherInstallationsSignatureText();
...
// Step 1: Generate the signature request
const signatureRequest =
  await client.unsafe_revokeAllOtherInstallationsSignatureRequest();
...
### Change recovery identifier signature request

Generate the signature request needed for changing the recovery identifier:
...
// Step 1: Generate the signature text for changing recovery
const { signatureText, signatureRequestId } =
...
// Step 2: Sign with the CURRENT recovery account's wallet
const currentRecoverySafeSigner = toSafeSigner(currentRecoveryWalletSigner);

// Step 3: Apply the signature
await client.unsafe_applySignatureRequest(currentRecoverySafeSigner, signatureRequestId);
```

```js [Node]
import { IdentifierKind } from "@xmtp/node-sdk";

// Step 1: Generate the signature request for changing recovery
const signatureRequest =
  await client.unsafe_changeRecoveryIdentifierSignatureRequest({
    kind: IdentifierKind.Ethereum,
    identifier: "0xNEW_RECOVERY_ADDRESS",
  });

// Step 2: Get the signature text and sign it with the CURRENT recovery account's wallet
const signatureText = signatureRequest.signatureText();
const signatureBytes = await currentRecoveryWallet.signMessage(signatureText);
