...
// map inbox ID to account identifiers
const inboxIdIdentifiersMap = new Map(
  members.map((member) => [member.inboxId, member.accountIdentifiers])
);

// find Ethereum address for a member
const ethIdentifier = members[0].accountIdentifiers.find(
