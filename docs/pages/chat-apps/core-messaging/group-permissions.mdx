...
**Note:** The `leaveGroup()` method is available in React Native SDK v5.1.0 and later.

:::code-group

```js [Browser]
// Not available in Browser SDK yet
```

```js [Node]
// Not available in Node SDK yet
```
...
// The group will no longer appear in active conversation lists
const activeGroups = await client.conversations.listGroups(
  { isActive: true },
  undefined,
  ['allowed']
);
// This group will not be included in activeGroups
```
