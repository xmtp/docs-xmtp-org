...
// Create a DM using an identifier instead of inbox ID
const dm = await client.conversations.createDmWithIdentifier({
  kind: IdentifierKind.Ethereum,
  identifier: "0xPEER_ADDRESS",
});
```
...
In rare cases, duplicate DM conversations can be created (for example, if two users simultaneously initiate a DM with each other). Use `duplicateDms()` to detect and handle these duplicates.
...
// Check each DM for duplicates
for (const dm of dms) {
  const duplicates = await dm.duplicateDms();
  if (duplicates.length > 0) {
    console.log(`Found ${duplicates.length} duplicate(s) for DM with ${dm.peerInboxId}`);
    // Handle duplicates - typically you'd want to consolidate messages
    // and archive or ignore the duplicate conversations
  }
}
```
