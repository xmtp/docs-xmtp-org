...
- `app_data`: Custom app-specific data for the group chat (max 8192 bytes)
...
Members can request to leave a group chat. This method initiates a request to remove the current user from the group.
...
## Check if pending removal from group

Check if the current user has a pending request to leave the group.
