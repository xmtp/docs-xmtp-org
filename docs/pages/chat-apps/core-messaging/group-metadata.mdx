...
Update the custom application-specific data for a group. The data is stored as a string with a maximum of 8192 bytes, so you'll need to serialize any objects (e.g., using JSON.stringify).
...
Members can request to leave a group. This creates a pending removal request that group admins can process.
...
Check if the current user has a pending removal request for the group.
...
```kotlin [Kotlin]
val isPending = group.isPendingRemoval()
```

```swift [Swift]
let isPending = try group.isPendingRemoval()
```
