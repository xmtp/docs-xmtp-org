...
For Browser SDK (v6.0.0+) and Node SDK (v5.0.0+), the reply content type is built-in and does not require separate installation or codec registration.

For other SDKs where the `ReplyCodec` is not already included, you can install the package using the following command:
...
if (message.contentType.sameAs(contentTypeReply())) {
  ...
  const originalMessage = message.inReplyTo?.content;
