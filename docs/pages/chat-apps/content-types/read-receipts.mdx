...
For Browser SDK (v6.0.0+) and Node SDK (v5.0.0+), the read receipt content type is built-in and does not require separate installation or codec registration.

For other SDKs, you can install the package using the following command:
...
```js [Node]
import { contentTypeReadReceipt } from '@xmtp/node-sdk';
