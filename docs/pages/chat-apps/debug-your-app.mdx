...
This document covers tools and features available for debugging apps built with XMTP, including stress testing, group chat diagnostics, logging, and network monitoring capabilities.
...
## Inspect duplicate DMs

XMTP automatically stitches duplicate DM conversations together at the protocol level. When two users simultaneously create a DM with each other, or when a user creates a new DM from a different installation before history sync completes, multiple underlying MLS groups can exist for the same pair of identities.

The SDK handles this transparently:

- **Listing**: When you call `list()` or `listDms()`, duplicate DMs are automatically filtered out. You only see one DM per peer.
- **Messages**: When you fetch messages from a DM, the SDK returns messages from all underlying MLS groups for that DM pair, so no messages are lost.
- **Sending**: All installations eventually converge to sending messages through the same underlying MLS group.

To learn more about how DM stitching works and its implications for push notifications, see [Understand DM stitching and push notifications](/chat-apps/push-notifs/understand-push-notifs#understand-dm-stitching-and-push-notifications).

### When to use `duplicateDms()`

The `duplicateDms()` method lets you inspect the duplicate DM conversations that exist under the hood but are normally hidden by automatic stitching. This is useful for:

- **Debugging**: Investigating unexpected message delivery behavior in DMs
- **Diagnostics**: Understanding how many underlying MLS groups exist for a DM pair
- **Push notification troubleshooting**: Verifying that all underlying topics are being subscribed to

The `duplicateDms()` method is currently only exposed as a public API in the Browser and Node SDKs. The iOS and Android SDKs use the equivalent `findDuplicateDms()` internally for push notification topic management, but don't expose it as a developer-facing method.
...
// Inspect underlying duplicate DMs for each conversation
for (const dm of dms) {
  ...
  if (duplicates.length > 0) {
    ...
    // These duplicates are automatically stitched together by the SDK.
    // Messages from all duplicates already appear in the main DM.
  }
}
```
...
const pausedVersion = conversation.pausedForVersion;
...
const groupPausedVersion = group.pausedForVersion;
const dmPausedVersion = dm.pausedForVersion;
```
...
const aggregateStats = await client.debugInformation.apiAggregateStatistics();
console.log(aggregateStats);

// Get individual API statistics
const apiStats = await client.debugInformation.apiStatistics();
console.log("Uploads:", apiStats.uploadKeyPackage);
console.log("Fetches:", apiStats.fetchKeyPackage);
console.log("Sent messages:", apiStats.sendGroupMessages);

// Get identity statistics
const identityStats = await client.debugInformation.apiIdentityStatistics();
console.log("Identity updates:", identityStats.publishIdentityUpdate);
console.log("Inbox ID lookups:", identityStats.getInboxIds);
