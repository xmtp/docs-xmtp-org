...
### Configure fork recovery policy

You can configure how the client handles fork recovery when creating the client. The `ForkRecoveryPolicy` determines which groups the client will attempt to recover from forks.

| Policy | Description |
| ------ | ----------- |
| `None` | No automatic fork recovery (default) |
| `AllowlistedGroups` | Only recover groups that the user has consented to |
| `All` | Attempt to recover all forked groups |
...
// Configure fork recovery when creating the client
val client = Client.create(
    wallet = wallet,
    options = ClientOptions(
        // ... other options
        forkRecoveryOptions = ForkRecoveryOptions(
            policy = ForkRecoveryPolicy.AllowlistedGroups
        )
    )
)
```
...
### Check conversation fork status

You can check the fork status of a specific conversation using the `commitLogForkStatus` property or method:

:::code-group

```kotlin [Kotlin]
// Check the fork status of a conversation
val forkStatus = conversation.commitLogForkStatus()

when (forkStatus) {
    CommitLogForkStatus.FORKED -> println("Conversation is forked")
    CommitLogForkStatus.NOT_FORKED -> println("Conversation is not forked")
    CommitLogForkStatus.UNKNOWN -> println("Fork status unknown")
}
```
...
Key packages are used by MLS for establishing secure group communications. You can check the status of key packages for specific installations to diagnose connection issues.

:::code-group

```kotlin [Kotlin]
// Check key package status for specific installations
val installationIds = listOf(installationId1, installationId2)
val keyPackageStatuses = Client.keyPackageStatusesForInstallationIds(
    installationIds = installationIds,
    api = client.apiClient
)

keyPackageStatuses.forEach { (installationId, status) ->
    println("Installation $installationId: $status")
}
```

:::

## Get newest message metadata

Retrieve metadata about the newest message in multiple groups at once. This is useful for efficiently updating UI indicators across many conversations.

:::code-group

```kotlin [Kotlin]
// Get newest message metadata for multiple groups
val groupIds = listOf(groupId1, groupId2, groupId3)
val messageMetadata = Client.getNewestMessageMetadata(
    groupIds = groupIds,
    api = client.apiClient
)

messageMetadata.forEach { (groupId, metadata) ->
    println("Group $groupId newest message: $metadata")
}
```
