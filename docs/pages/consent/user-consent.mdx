...
Support user consent preferences in your app to provide your users with spam-free inboxes.
...
1. `alix.eth` starts a conversation with `bo.eth`. At this time, `alix.eth` is unknown to `bo.eth` and you can display this conversations in a requests UI.

    <div>
    <img src="https://raw.githubusercontent.com/xmtp/docs-xmtp-org/main/docs/pages/img/requests-tab.png" width="375px" />
    </div>

2. `bo.eth` views the message request. You can give `bo.eth` the option to **Block** or **Accept** the contact.

    <div>
    <img src="https://raw.githubusercontent.com/xmtp/docs-xmtp-org/main/docs/pages/img/block-accept.jpg" width="375px" />
    </div>

   - If `bo.eth` accepts `alix.eth` as a contact, display their conversation in `bo.eth`'s main inbox. Because only contacts `bo.eth` accepts display in their main inbox, their inbox remains spam-free.
