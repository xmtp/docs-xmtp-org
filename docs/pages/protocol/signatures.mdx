...
Learn about the types of account signatures you might be prompted to provide when using apps built with XMTP (Extensible Message Transport Protocol).

## Sign to create an inbox (first-time use)

The first time you use an app built with XMTP, an "XMTP : Authenticate to inbox" window displays. It requests that you sign a "Create inbox" message with your wallet account. Sign the message with your wallet account to create an XMTP inbox owned by your wallet account.
...
If you decide to add another wallet account to your inbox, an "XMTP : Authenticate to inbox" window displays. It requests that you sign a "Link address to inbox" message. Sign with the wallet account you want to add to grant it access to the inbox. You can now use your inbox to exchange messages using the wallet account you just added, as well as any existing wallet accounts.

## Sign to remove account from your inbox

You can remove a wallet account from your inbox at any time.

If you decide to remove a wallet account from your inbox, an "XMTP : Authenticate to inbox" window displays. It requests that you sign an "Unlink address from inbox" message. Sign with the wallet account you want to remove to unlink it from your inbox. You can no longer access your inbox using the wallet account you removed.

## Sign to change inbox recovery address

The first time you use an app built with XMTP, the wallet account you use to create an inbox is automatically set as the inbox recovery address. You can change the recovery address at any time.

If you decide to change the recovery address, an "XMTP : Authenticate to inbox" window displays. It requests that you sign a "Change inbox recovery address" message. Sign with the wallet account you want to set as the recovery address to change the recovery address.
...
For example, here’s the MetaMask **Signature request** window that displays when clicking the **Subscribe** button on this [example subscription page](https://subscribe-broadcast.vercel.app/subscribe/button) connected to the XMTP `dev` network. You will typically see **Subscribe** buttons like this on a web page or in a dapp.
