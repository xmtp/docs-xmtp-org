...
- **Usage**: When an app wants to receive messages for a conversation, it subscribes to this topic. The `conversation.topic` property in the SDKs provides this value (available in Node SDK v5.4.0+ and Browser SDK v6.4.0+).
...
- **Usage**: A push notification server subscribes to an installation's welcome topic to be notified when that installation is invited to a new group chat or DM. The SDKs provide this via the `client.conversations.topic` property (available in Node SDK v5.4.0+ and Browser SDK v6.4.0+).
