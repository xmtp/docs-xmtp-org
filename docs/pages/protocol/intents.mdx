...
An intent represents an action that intends to change the state of a group chat via a commit, along with enough information to retry the action if it fails.

Each commit rotates the group's encryption state into a new epoch and therefore must be strictly ordered. If commits arrive out of order or conflict, groups can fork, making some members unable to decrypt other members' messages.
...
- **Error**: Intent failed (e.g., permission denied, removed from the group)
  - Retryable error: Can be retried on the next sync. For example, a temporary failure to publish the commit.
  - Non-retryable error: Can't be retried and moves the intent to the **Error** state. For example, a member without adequate permission tries to add a member.
