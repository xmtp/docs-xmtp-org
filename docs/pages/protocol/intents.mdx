...
Each commit rotates the group's encryption state into a new [epoch](/protocol/epochs) and therefore must be strictly ordered. If commits arrive out of order or conflict, groups can fork, making some members unable to decrypt other members' messages.
