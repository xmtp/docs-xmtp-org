...
- **`XMTP_ENV`**: The XMTP network your agent connects to:
  - `local` — For running against a local XMTP backend (i.e. Docker container)
  - `dev` — For using the XMTP test network
  - `production` — For making your agent discoverable on production apps like [Base](https://base.app/) or [World](https://world.org/)
