...
agent.on("text", async (ctx) => {
  ...
  // Send a message
  await conversation.send("Hello from the conversation!");
...
agent.on("text", async (ctx) => {
  ...
  const newDM = await conversations.newDm("target-inbox-id");
  const newGroup = await conversations.newGroup(["inbox1", "inbox2"], {
    groupName: "New Group"
  });
});
```
...
agent.on("text", async (ctx) => {
  try {
    ...
    await ctx.sendTextReply("Sorry, I encountered an error processing your message.");
  }
});
```
...
agent.on("group", async (ctx) => {
  ...
  const members = await ctx.conversation.members();
  await ctx.sendTextReply(`Hello everyone! I see there are ${members.length} members here.`);
});
```
...
1. **Use helper methods** – They provide cleaner, more readable code
2. **Extract sender info early** – Get sender address once and reuse
3. **Handle missing data** – Check for undefined values in message content
4. **Leverage conversation type** – Use DM vs group specific properties
5. **Cache expensive operations** – Store member lists or message history when needed
...
agent.on("text", async (ctx) => {
  const userId = ctx.getSenderAddress();
  const currentState = userStates.get(userId) || "idle";
  
  // State machine logic using context
  switch (currentState) {
    case "idle":
      if (ctx.message.content === "/start") {
        userStates.set(userId, "collecting_info");
        await ctx.sendTextReply("What's your name?");
      }
      break;
      
    case "collecting_info":
      userStates.set(userId, "idle");
      await ctx.sendTextReply(`Nice to meet you, ${ctx.message.content}!`);
      break;
  }
});
```
...
```typescript
agent.on("text", async (ctx) => {
  if (ctx.conversation.peerInboxId) {
...
- Learn about [event-driven architecture](./event-driven) for overall patterns
- Explore [message filters](./filters) for selective processing
- See [middleware and routing](./middleware) for request flow control
