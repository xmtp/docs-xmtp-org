...
agent.on("text", async (ctx) => {
  ...
  if (filter.fromAddress(ctx.message, "0x1234...")) {
    await ctx.conversation.send("Hello, authorized user!");
  }
});
```
...
agent.on("text", async (ctx) => {
  if (isDM(ctx.conversation)) {
    await ctx.conversation.send("Thanks for the DM!");
  }
...
agent.on("text", async (ctx) => {
  if (isRecent(ctx.message, 1)) {
    await ctx.conversation.send("Thanks for the fresh message!");
  }
  
  if (!isDuringBusinessHours(ctx.message)) {
    await ctx.conversation.send("Thanks for your message! I'll respond during business hours.");
  }
});
```
