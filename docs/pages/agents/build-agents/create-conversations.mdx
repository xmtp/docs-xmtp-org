...
const response = await Agent.canMessage(identifiers);
```
...
const conversationById = await agent.client.conversations.getConversationById(
  conversationId
);
...
const dmByInboxId = await agent.client.conversations.getDmByInboxId(peerInboxId);
```
