...
Roles that can be assigned to each participant (inbox ID) in a group chat:

- **Member** - Everyone in a group chat. Can be granted admin or super admin status.
- **Admin** - Members with elevated permissions
- **Super admin** - Highest permission level

How to check and manage group chat member statuses:
