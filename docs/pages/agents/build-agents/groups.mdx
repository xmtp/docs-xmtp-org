...
Request to leave the group chat:

```js [Node]
await group.requestRemoval();
```

### Check pending removal status

Check if the current user has requested to leave the group:

```js [Node]
const isPending = group.isPendingRemoval;
if (isPending) {
  console.log('User has requested to leave this group');
}
```
