...
- **Database persistence is mandatory** - Use persistent volumes (Railway volumes, Render disks, etc.)
- The database must persist across restarts to avoid creating new installations
- **Never rely on creating new installations as a deployment strategy** - This will cause your agent to fail when you hit the 10 installation limit
