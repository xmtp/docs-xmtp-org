...
registerAction("get-info", async (ctx) => {
  await ctx.conversation.send("Here's some information about the agent...");
  ...
  await sendSelection(ctx, "Choose a setting to change:", [
    {
      ...
      label: "🎨 Theme",
      handler: async (ctx) => {
        await ctx.conversation.send("Theme settings would go here...");
      },
    },
    {
      id: "notifications",
      label: "🔔 Notifications",
      handler: async (ctx) => {
        await ctx.conversation.send("Notification settings would go here...");
      },
    },
  ]);
});
...
registerAction("delete-item", async (ctx) => {
  await sendConfirmation(
    ...
    async (ctx) => {
      await ctx.conversation.send("Item deleted successfully!");
    ...
    async (ctx) => {
      await ctx.conversation.send("Delete cancelled.");
...
agent.on("text", async (ctx) => {
  ...
  if (message.includes("menu") || message.includes("start")) {
    ...
    await ctx.conversation.send("I received your message! Here are some options:");
...
1. **Keep menus simple**: Don't overwhelm users with too many options
2. **Provide clear labels**: Use descriptive button text
3. **Handle errors gracefully**: Always provide fallback options
4. **Use appropriate button types**: Use danger buttons for destructive actions
5. **Provide navigation**: Always give users a way to go back or get help
6. **Test interactions**: Ensure all button combinations work correctly
