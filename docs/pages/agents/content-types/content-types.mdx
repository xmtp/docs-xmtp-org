...
agent.on('attachment', async (ctx) => {
  console.log(`Received attachment: ${JSON.stringify(ctx.message.content)}`);
  // Handle attachment logic
});

agent.on('reaction', async (ctx) => {
  console.log(`Received reaction: ${JSON.stringify(ctx.message.content)}`);
  // Handle reaction logic
});

agent.on('reply', async (ctx) => {
  console.log(`Received reply: ${JSON.stringify(ctx.message.content)}`);
  // Handle reply logic
});

agent.on('transaction', async (ctx) => {
  console.log(`Received transaction: ${JSON.stringify(ctx.message.content)}`);
...
agent.on('message', async (ctx) => {
  ...
  if (contentType?.typeId === 'my-custom-type') {
    console.log(`Received custom content: ${JSON.stringify(ctx.message.content)}`);
  }
});
```
...
- [Group updates content type](/agents/content-types/group-update): Use to send group updates, such as name, description, and members.
