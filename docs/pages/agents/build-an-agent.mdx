...
When you revoke an agent installation, it can no longer send or receive messages. However, you can still access the local database, if its not encrypted. You can also still access the message history up to the point that the installation was revoked.
