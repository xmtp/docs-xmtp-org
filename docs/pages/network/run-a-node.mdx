...
- **Early January 2026**: Migration testing

  These [three registered nodes](https://status.testnet.xmtp-partners.xyz/) are testing the processes that will migrate **150M+ real messages** from XMTP's V3 network to the decentralized XMTP Mainnet. These three nodes were the first to be on xmtpd v1.0.0.

  Being able to migrate these messages ensures a smooth transition for app devs by making previously-published messages available on day 1 of the decentralized Mainnet.

- **Late January 2026**: Load testing

  After migration testing is complete, an expanded group of [Testnet node operators](/network/network-nodes) will be able to migrate V3 messages and participate in expanded load testing.
...
9. **Catch up with Testnet with migrated V3 messages**

   Node must be fully caught up with Testnet data that includes the migrated V3 messages.
