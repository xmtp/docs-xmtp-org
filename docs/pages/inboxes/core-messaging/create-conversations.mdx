...
To learn more about optimistically sending messages using `prepareMessage()` and `publishMessages()`, see [Optimistically send messages](/inboxes/core-messaging/send-messages#optimistically-send-a-message).
