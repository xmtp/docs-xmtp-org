...
History sync is enabled by default and runs automatically. When [creating a client](#TODO), the `historySyncUrl` client option is dynamically set to a default history sync server URL based on your `env` client option setting.
