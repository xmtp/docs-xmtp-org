...
Listens to the network for new group chats and DMs. Whenever a new conversation starts, it triggers the provided callback function with a [`ConversationContainer` object](#TODO). This allows the client to immediately respond to any new group chats or DMs initiated by other users.
