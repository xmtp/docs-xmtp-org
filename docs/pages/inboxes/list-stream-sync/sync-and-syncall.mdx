...
Sync all new welcomes, group chat and DM conversations, messages, and [preference updates](#TODO) from the network.
...
To sync preferences only, you can call [`preferences.sync`](#TODO). Note that `preferences.sync` will also sync welcomes to ensure that you have all potential new installations before syncing.
