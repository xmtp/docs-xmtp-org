...
```jsx [React Native]
await client.revokeInstallations([installationIds], signer.getIdentifier())
```
...
```jsx [React Native]
await client.revokeAllOtherInstallations(signer.getIdentifier())
```
...
Static installation revocation enables users to revoke installations without needing to log in or have access to their installations. XMTP React Native SDK v4.2.5 includes improvements to statically manage revoking installation signatures, enhancing the reliability of this process. 
...
```jsx [React Native]
import { staticInboxStatesForInboxIds, staticRevokeInstallations } from '@xmtp/react-native-sdk'

const states = await staticInboxStatesForInboxIds('production', [inboxId])

const toRevokeIds = states[0].installations.map((i) => i.id)

await staticRevokeInstallations(
  'production',
  recoveryWallet.getIdentifier(),
  inboxId,
  toRevokeIds
)
```
...
```jsx [React Native]
import { getOrCreateInboxId } from '@xmtp/react-native-sdk'

const inboxId = await getOrCreateInboxId(publicIdentity, client.environment)
```
...
```jsx [React Native]
import { getInboxState, getInboxStates } from '@xmtp/react-native-sdk'

const state = await getInboxState(client.installationId, true)
const states = await getInboxStates(client.installationId, true, [inboxId, inboxId])
```
...
```jsx [React Native]
import { addAccount } from '@xmtp/react-native-sdk'

await addAccount(client.installationId, newIdentity)
```
...
```jsx [React Native]
import { removeAccount } from '@xmtp/react-native-sdk'

await removeAccount(client.installationId, identityToRemove, recoveryIdentity)
```
