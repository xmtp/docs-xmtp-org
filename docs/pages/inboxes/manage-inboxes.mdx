...
3. If there are more than two identities, the first non-recovery identity in the ordered array is typically the preferred display identity.

:::code-group

```tsx [Browser]
const inboxState = await client.preferences.inboxStateFromInboxIds([inboxId]);
const identities = inboxState[0].identities;
const recoveryIdentity = inboxState[0].recoveryIdentity;

// Get the preferred display identity
let displayIdentity = identities[0].identifier;

// If there are multiple identities and the first one is the recovery identity,
// use the second identity as the display identity
if (identities.length > 1 && identities[0].identifier === recoveryIdentity) {
  displayIdentity = identities[1].identifier;
}
```
