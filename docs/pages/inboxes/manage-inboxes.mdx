...
The client creates an inbox ID and installation ID associated with the identity. Each XMTP client installation (on a device or in a browser) is assigned a unique installation ID, which identifies a specific instance of the XMTP client.

By default, this identity is designated as the recovery identity. A recovery identity will always have the same inbox ID and cannot be reassigned to a different inbox ID.
