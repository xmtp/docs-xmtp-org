...
You can revoke a specific installation other than the currently accessed installation. You can use this function to revoke an installation when an inbox ID has reached its maximum of 5 installations.
...
// revoke specific installation(s)
...
```jsx [React Native]
// Revoke all the other installations but this one
await client.revokeAllOtherInstallations(signingKey)
// Revoke individual installations based on id
