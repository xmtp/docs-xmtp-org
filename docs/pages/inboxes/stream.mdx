...
await alix.conversations.stream(
  ...
  "all", // Optional: filter by type ("all", "groups", "dms")
  () => {
    // Optional: callback when stream is closed
    console.log("Conversation stream closed");
  }
);
```
...
await alix.conversations.streamAllMessages(
  ...
  "all", // Optional: filter by type ("all", "groups", "dms")
  ["allowed"], // Optional: filter by consent state
  () => {
    // Optional: callback when stream is closed
