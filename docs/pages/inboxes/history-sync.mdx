...
When your app initializes an XMTP client and a `deviceSyncUrl` client option is present, device sync automatically triggers an initial sync request and creates an encrypted payload.

<div>
    <img src="https://raw.githubusercontent.com/xmtp/docs-xmtp-org/refs/heads/main/docs/pages/img/device-sync-1.png" width="600px" />
</div>

Device sync then uploads the payload, sends a sync reply, and pulls all conversation state history into the new device, merging it with the existing devices in the device group.
...
- Device group
- The device sync worker (A worker thread in libXMTP)
- Device sync server
