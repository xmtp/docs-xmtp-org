...
Ongoing HMAC and consent updates are streamed automatically across the sync group, ensuring all app installations have up-to-date preferences.
...
A device group is a private, hidden [Messaging Layer Security](/protocol/security) sync group that includes all of the user's devices. The device group is used to send serialized sync messages of a local device state across a user's other devices. This state information includes:
...
A device sync worker is a worker thread in libXMTP. This spawned background worker listens for sync events and processes them accordingly. This worker:
...
An archive server acts as a bucket that holds encrypted sync payloads. The URL location of these payloads and the password (cipher encryption key) to decrypt these payloads is sent to the device group for the recipient to decrypt.

We should mention that files only live for a day before they're deleted.
