...
As soon as your app receives a user preference update event from [preferences.sync](#TODO) or [preferences.stream](#stream-hmac-key-update-events) indicating new HMAC keys for a user, resubscribe to topics to get the new HMAC keys. For example:
