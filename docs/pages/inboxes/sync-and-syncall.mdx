...
By default, `syncAll` streams only conversations with a [consent state](/inboxes/user-consent/user-consent#how-user-consent-preferences-are-set) of allowed or denied.

We recommend syncing messages for allowed conversations only. This ensures that spammy conversations with a consent state of unknown don't degrade the user experience.
...
```tsx [React Native]
await client.conversations.syncAllConversations([ConsentState.ALLOWED]);
```

```kotlin [Kotlin]
client.conversations.syncAllConversations(consentState: [ALLOWED])
```
