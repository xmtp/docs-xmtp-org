...
Sync all new welcomes, group chat and DM conversations, messages, and installation-related updates (consent states, HMAC keys, etc.) from the network.
...
We recommend syncing messages for allowed conversations only. This ensures that spammy conversations with a consent state of unknown don't degrade the user experience. Or store unwanted spam messages in the users local database.
