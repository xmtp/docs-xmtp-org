...
When you pass `allowOffline = true` to [`Client.create()`](#create-a-client), or build a client with [`Client.build()`](#build-an-existing-client) and a locally stored `inboxId`, the client operates in offline mode as follows:

- Skips all network requests (identity ledger, key packages, etc.)
- Works entirely from the local database
- Postpones identity registration if no inbox ID exists
...
You can pass in `allowOffline = true` to create a client in [offline mode](#offline-mode).
...
When building a client with an existing `inboxId`, [offline mode](#offline-mode) is automatically enabled since no network request is needed to check the identity ledger.
