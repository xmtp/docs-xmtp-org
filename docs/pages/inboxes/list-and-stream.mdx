...
Get a list of existing conversations in the local database.
...
We recommend streaming messages for `allowed` conversations `[Allowed]` only. This ensures that spammy conversations with a consent state of `unknown` don't take up networking resources.
