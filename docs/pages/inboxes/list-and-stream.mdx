...
By default, `list` returns only conversations with a [consent state](/inboxes/user-consent/user-consent#how-user-consent-preferences-are-set) of allowed or denied.
...
```js [Browser]
const allConversations = await client.conversations.list([ConsentState.ALLOWED]);
const allGroups = await client.conversations.listGroups([ConsentState.ALLOWED]);
const allDms = await client.conversations.listDms([ConsentState.ALLOWED]);
```
...
await alix.conversations.list([ConsentState.ALLOWED]);
...
Listens for new messages on the network in all active group chat and DM conversations that were created after the stream started.
...
By default, `streamAll` streams only conversations with a [consent state](/inboxes/user-consent/user-consent#how-user-consent-preferences-are-set) of allowed or denied.

We recommend streaming messages for allowed conversations only. This ensures that spammy conversations with a consent state of unknown don't take up networking resources.
...
```js [Browser]
const stream = await client.conversations.streamAllMessages([ConsentState.ALLOWED]);
...
const stream = await client.conversations.streamAllGroupMessages([ConsentState.ALLOWED]);
 
// stream only dm messages
const stream = await client.conversations.streamAllDmMessages([ConsentState.ALLOWED]);
