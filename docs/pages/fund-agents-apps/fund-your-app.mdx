...
1. Use your payer wallet to connect to the XMTP Funding Portal → testnet-staging link coming soon.
...
:::warning[Using testnet-staging?]

If you are using the XMTP Funding Portal on testnet-staging, you must use mxUSD, not USDC. To get mxUSD, you must self-mint it. To learn how, see [For use with testnet-staging only: Self-mint mxUSD](#for-use-with-testnet-staging-only-self-mint-mxusd).
...
To fund your app on the testnet-staging environment, you must use mxUSD instead of USDC. To get mxUSD, you must self-mint it.
...
2. Use the `xmtpd-cli` Docker image with your relevant parameter values to self-mint mxUSD:
   ...
   ```bash
    docker run --rm ghcr.io/xmtp/xmtpd-cli:main \
      ...
      --private-key <receiving-address-private-key> \
...
To learn how to create an Alchmey API endpoint, see [Create an Alchemy API Key](https://www.alchemy.com/docs/create-an-api-key).
