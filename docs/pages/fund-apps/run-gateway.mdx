...
You host your XMTP Gateway on your infrastructure because it contains sensitive information such as your Payer Private Key.

XMTP provides a standard implementation of an XMTP Gateway that you can extend to meet your unique needs. Once your XMTP Gateway is deployed, you will need to include the `payer_service_address` when creating any XMTP client in your app or agent.

## Get started

**Every app needs a XMTP Gateway**.

### For Node.js apps

The XMTP Gateway is built into the XMTP Node SDK. No need to run a separate XMTP Gateway.
...
You must run the XMTP Gateway written in Go. Choose the option that works best for you:
...
The private key for your XMTP Gateway is an Ethereum account that is expected to be funded in the XMTP Funding Portal.

To learn more, see [Funding your XMTP Gateway](fund-apps/quickstart).

## Authenticate users

You can think of the XMTP Gateway as an extension of your client app. If you already have a system for authenticating user requests in your app, you should use it in your XMTP Gateway. For example, many apps use [JSON Web Tokens](https://jwt.io) (JWTs) to authenticate client requests to servers.

You can use any authentication scheme you like to authenticate requests from your client to the XMTP Gateway.

:::tip[IMPORTANT]

Browser and mobile client apps that need user authentication must customize the XMTP Gateway to integrate with their authentication system. Node.js apps that need user authentication can use the XMTP Gateway provided by the XMTP Node SDK without customization.
...
## Deploy your XMTP Gateway

Deploy the XMTP Gateway on your infrastructure of choice, such as a container hosting service ($25-50/month minimum).
...
To get testnet USDC, [use this faucet](#TODO) or DM `somesupportensxxxxxx.eth` for bulk credits.
...
The following sequence diagrams illustrate the XMTP Gateway's role in the following key flows.
